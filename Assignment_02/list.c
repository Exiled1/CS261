...
/**
 * This function simply returns the value in the head of the list.
 * @param list
 * @return void* to the value of the list head.
 */
void *list_get_top_val(struct list *list) {
    return list->head->val;
}

/**
 * Removes the head, then moves the head pointer to the next element. Then returns the value of the popped head.
 * @param list
 */
void *list_remove_head(struct list *list) {

    struct link *temp;
    void *returnVal = list->head->val;
    temp = list->head->next;
    free(list->head);
    list->head = temp;
    return returnVal;
...
/**
 * Will return a 1 if the list is null, 0 otherwise.
 * @param list
 * @return
 */
